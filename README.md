...
心潮是一个独立、可自托管的 AI 动态状态层。它在 wake 或外部事件到来时观察现实、评估驱动力并记录结果，同时维护睡眠、梦境余韵与短期窗口状态；没有事件时，wall-clock 不会自行累加主观 drive。
...
- 可选 LMC-5 bridge 或 Ombre-compatible 长期记忆 MCP；已完成的重要行动以稳定 external_id 回流为候选经验。
- 可选 Bark 通知与跨类型去重。
- 原子状态持久化与结构化转换日志。
